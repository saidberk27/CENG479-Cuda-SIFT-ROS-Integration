...
// Local
#include "parallel/cuda_kernels.hpp"
#include "parallel/gpu_feature_bus.hpp"
#include "parallel/perf_tracker.hpp"
...
public:
        ...
        : Node("sift_pipeline")
    {
        ...
        declare_parameter("output_dir",
                          (fs::current_path() / "src" / "parallel" / "output").string());
...
private:
    ...
    void produce(const std::string &path,
                 parallel::GpuFeatureBus::Slot slot,
                 ...
                 int half_cores)
    {
        ...
        auto gf = parallel::upload_features_to_gpu(
    ...
    void run_once()
    {
        ...
        // ── Stage A: parallel produce (SIFT + upload + publish) ──────────────
        {
            ...
            std::thread t_query([&]
                                { produce(query_path_, parallel::GpuFeatureBus::Slot::kQuery,
                                          "query", half); });
            std::thread t_large([&]
                                { produce(large_path_, parallel::GpuFeatureBus::Slot::kLarge,
                                          "large", half); });
            t_query.join();
            t_large.join();
        }
        ...
        std::shared_ptr<parallel::GpuFeatures> q, l;
        {
    ...
    std::unique_ptr<parallel::PerfTracker> perf_;
    std::mutex perf_mtx_, vis_mtx_;

    parallel::GpuFeatureBus bus_;
